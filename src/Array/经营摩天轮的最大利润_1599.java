...
public class 经营摩天轮的最大利润_1599 {
    ...
    public static int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        int index = 0;
        int peopleNum = 0;
        while (peopleNum > 0 || index < customers.length) {
            peopleNum = peopleNum + (index < customers.length ? customers[index] : 0);
            int up = Math.min(4, peopleNum);
            peopleNum = peopleNum - up;
            value = value + (up * boardingCost - runningCost);
            index++;
            if (value > maxValue) {
                maxValue = value;
                minCount = index;
